<!--
Local metadata preserved from pre-pull work:
Resource name: nic-testes
Latest Deployment Date: 2026-05-14 13:06:33
-->
...
O NIC recebe notificações em tempo real do Chatwoot via webhooks para sincronizar eventos como novas mensagens, mudanças de status de conversas, etc.

#### Configuração

1. **URL do Webhook**: `POST /api/webhooks/chatwoot`
2. **Eventos suportados**:
   - `message_created` - Nova mensagem recebida
   - `message_updated` - Mensagem editada
   - `conversation_created` - Nova conversa iniciada
   - `conversation_updated` - Conversa atualizada
   - `conversation_status_changed` - Status alterado (aberta/resolvida/pendente)

3. **Validação de Segurança** (opcional):
   - Configure `CHATWOOT_WEBHOOK_SECRET` no `.env`
   - O Chatwoot enviará assinatura HMAC-SHA256 no header `X-Chatwoot-Signature`
   - Se não configurada, aceita qualquer requisição (modo desenvolvimento)

#### Como configurar no Chatwoot

1. Acesse **Settings > Webhooks** no Chatwoot
2. Clique em **Add webhook**
3. **URL**: `https://seudominio.com/api/webhooks/chatwoot`
4. **Webhook Secret**: Defina uma chave secreta (mesma do `.env`)
...
```bash
php concilia-backend/scripts/test_chatwoot_webhook.php
```

Ou teste manualmente com curl:

```bash
curl -X POST http://localhost:8123/api/webhooks/chatwoot \
  -H 'Content-Type: application/json' \
  -d '{"event":"message_created","content":"Teste","conversation":{"id":123}}'
```

#### Logs e Debug

Os webhooks são logados em `storage/logs/laravel.log`. Para debug:

```bash
tail -f concilia-backend/storage/logs/laravel.log | grep webhook
```

O último evento fica em cache para consulta:

```php
// No tinker
Cache::get('chatwoot:webhook:last_event')
```
