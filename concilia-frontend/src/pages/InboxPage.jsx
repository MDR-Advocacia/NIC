...
const InboxPage = () => {
  ...
  return (
    <div style={{ display: 'flex', height: '88vh', backgroundColor: '#f0f2f5', margin: '-20px' }}>
      ...
      {/* SUBMENU LATERAL (ABAIXO DA CAIXA DE ENTRADA NO NIC) */}
      <div style={{ width: '220px', backgroundColor: '#fff', borderRight: '1px solid #ddd', padding: '10px' }}>
        <div style={{ fontWeight: 'bold', color: '#007bff', marginBottom: '15px', padding: '10px', backgroundColor: '#eef6ff', borderRadius: '8px' }}>
          📂 Caixa de Entrada
        </div>
        <div 
          onClick={() => setVisaoAtiva('conversas')}
          style={{ padding: '10px', cursor: 'pointer', borderRadius: '6px', backgroundColor: visaoAtiva === 'conversas' ? '#f0f0f0' : 'transparent', fontSize: '14px', marginBottom: '5px' }}
        >
          💬 Conversas
        </div>
        <div 
          onClick={() => setVisaoAtiva('contatos')}
          style={{ padding: '10px', cursor: 'pointer', borderRadius: '6px', backgroundColor: visaoAtiva === 'contatos' ? '#f0f0f0' : 'transparent', fontSize: '14px', display: 'flex', justifyContent: 'space-between' }}
        >
          <span>👥 Contatos</span>
          <span onClick={(e) => { e.stopPropagation(); setModalAberto(true); }} style={{ color: '#25D366', fontWeight: 'bold' }}>+</span>
        </div>
      </div>
      ...
      <div style={{ width: '350px', backgroundColor: '#fff', borderRight: '1px solid #ddd', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '15px', borderBottom: '1px solid #eee' }}>
          {visaoAtiva === 'conversas' ? (
            <>
              <select value={inboxSelecionada} onChange={(e) => setInboxSelecionada(e.target.value)} style={{ width: '100%', padding: '8px', marginBottom: '10px', borderRadius: '6px' }}>
          ...
          {visaoAtiva === 'conversas' ? (
             conversas.filter(chat => inboxSelecionada === 'all' || chat.inbox_id == inboxSelecionada).map(chat => (
               <div key={chat.id} onClick={() => abrirConversa(chat.id)} style={{ padding: '12px 15px', borderBottom: '1px solid #f5f5f5', cursor: 'pointer', backgroundColor: conversaSelecionada === chat.id ? '#f0f7ff' : '#fff', display: 'flex', alignItems: 'center', gap: '12px' }}>
                 <div style={{ width: '45px', height: '45px', borderRadius: '50%', backgroundColor: '#6c757d', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', overflow: 'hidden' }}>
                 ...
                 <div style={{ flex: 1, minWidth: 0 }}>
                   <div style={{ fontSize: '14px', fontWeight: chat.unread_count > 0 ? 'bold' : '500', display: 'flex', justifyContent: 'space-between' }}>
                     <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{chat.meta?.sender?.name}</span>
            ...
            contatos.filter(c => c.name.toLowerCase().includes(buscaContato.toLowerCase())).map(c => (
              <div key={c.id} style={{ padding: '12px 15px', borderBottom: '1px solid #f5f5f5', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <button onClick={() => iniciarConversa(c.id)} style={{ backgroundColor: '#25D366', color: '#fff', border: 'none', padding: '5px 8px', borderRadius: '5px', cursor: 'pointer', fontSize: '10px' }}>Enviar</button>
              </div>
            ))
          )}
        </div>
              ...
              {mensagens.map((m, i) => {
                ...
                return (
                  <div key={i} style={{ display: 'flex', flexDirection: eMinha ? 'row-reverse' : 'row', alignItems: 'flex-end', marginBottom: '12px', gap: '8px' }}>
                    <div style={{ width: '32px', height: '32px', borderRadius: '50%', backgroundColor: eMinha ? '#007bff' : '#6c757d', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', overflow: 'hidden' }}>
                      {m.sender?.avatar_url ? <img src={m.sender.avatar_url} style={{width:'100%', height:'100%', objectFit:'cover'}} /> : m.sender?.name?.charAt(0)}
                    </div>
