...
class LegalCaseController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        // Filtro por Status (singular)
        if ($request->has('status') && $request->input('status') != '') {
            $query->where('status', $request->input('status'));
        }

        // Filtro por Status (múltiplos via statuses[])
        ...
        // Filtro por Indicadores (múltiplos via indicator_user_ids[])
    ...
    public function update(Request $request, LegalCase $case): JsonResponse
    {
        ...
        // Validação obrigatória para Acordo Fechado e Fechado em Audiência
        $closingStatuses = [LegalCase::STATUS_CLOSED_DEAL, LegalCase::STATUS_CLOSED_IN_HEARING];
        $newStatus = $validatedData['status'] ?? $case->status;
        if (in_array($newStatus, $closingStatuses, true) && !in_array($case->status, $closingStatuses, true)) {
            $agreementValue = $validatedData['agreement_value'] ?? $case->agreement_value;
            $agreementDate = $validatedData['agreement_closed_at'] ?? $case->agreement_closed_at;
            $errors = [];
            if (empty($agreementValue) || (float) $agreementValue <= 0) {
                $errors['agreement_value'] = 'O valor do acordo é obrigatório para fechar o acordo.';
            }
            if (empty($agreementDate)) {
                $errors['agreement_closed_at'] = 'A data do acordo é obrigatória para fechar o acordo.';
            }
            if (!empty($errors)) {
    ...
    public function batchUpdate(Request $request): JsonResponse
    {
        ...
        $closingStatuses = [LegalCase::STATUS_CLOSED_DEAL, LegalCase::STATUS_CLOSED_IN_HEARING];
        if ($action === 'update_status' && in_array($value, $closingStatuses, true)) {
            $missing = LegalCase::whereIn('id', $caseIds)
                ->whereNotIn('status', $closingStatuses)
                ->where(function ($q) {
                    $q->whereNull('agreement_value')
                      ->orWhere('agreement_value', '<=', 0)
                      ->orWhereNull('agreement_closed_at');
                })
                ->count();
            if ($missing > 0) {
                throw ValidationException::withMessages([
                    'agreement_value' => "{$missing} caso(s) não possuem valor do acordo e/ou data do acordo preenchidos. Preencha antes de mover para acordo fechado.",
                ]);
            }
        }
